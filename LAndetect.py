...
def canny(image): # opencv ka canny feature use kiya hai. After blurrring, so that noise na aaye.
...
def display_line(image, lines):#To display lines on the image/vid, taki we can see VISUALLY TOO.
...
cap = cv2.VideoCapture("test2.mp4") #Video time, actually seeing if it is working on a video, not just image , now we just have to add a live feed from camera.
while (cap.isOpened()):
    _, frame = cap.read()
